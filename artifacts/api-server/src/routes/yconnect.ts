...
const cases = [
  [1, "YC-000123", "Case 123", "prevention", "Kumasi", "active", "moderate", "Efua Boateng"],
  [2, "YC-000124", "Case 124", "alhiv", "Kumasi", "re_engaged", "elevated", "Efua Boateng"],
  [3, "YC-000125", "Case 125", "pregnant", "Kumasi", "active", "moderate", "Akosua Antwi"],
  [4, "YC-000126", "Case 126", "caregiver", "Kumasi", "active", "elevated", "Akosua Antwi"],
  [5, "YC-000127", "Case 127", "prevention", "Accra", "active", "low", "Yaw Asare"],
  [6, "YC-000128", "Case 128", "alhiv", "Accra", "active", "urgent", "Yaw Asare"],
  [7, "YC-000129", "Case 129", "pregnant", "Kumasi", "active", "elevated", "Akosua Antwi"],
  [8, "YC-000130", "Case 130", "caregiver", "Accra", "closed", "low", "Nana Kusi"],
  [9, "YC-000131", "Case 131", "prevention", "Kumasi", "active", "moderate", "Efua Boateng"],
  [10, "YC-000132", "Case 132", "alhiv", "Accra", "re_engaged", "elevated", "Yaw Asare"],
].map(([id, caseRef, alias, subtype, district, status, riskLevel, assignedTo]) => ({ id, caseRef, alias, subtype, district, status, riskLevel, assignedTo }));
...
const notifications = [
  { id: 1, title: "Follow-up due today", body: "YC-000123 has a task due today.", unread: true },
  { id: 2, title: "New connection request", body: "A new request is waiting to be assigned.", unread: true },
  { id: 3, title: "Milestone reminder", body: "INF-000047 has a milestone coming up.", unread: false },
];
...
function requireStaff(req: Request, res: any, permission?: string): User | null {
...
router.get("/content", (req, res) => {
  ...
  res.json(category ? content.filter((item) => item.category === category) : content);
});

router.get("/content/:slug", (req, res) => {
  const item = content.find((entry) => entry.slug === req.params.slug);
...
router.post("/assessments", (req, res) => {
  ...
  const scores: Record<string, number> = { condomless_sex: 3, multiple_partners: 2, partner_status_unknown: 2, sti_symptoms: 2, injection_sharing: 3, forced_sex: 3, known_positive_partner: 3, pregnant: 1 };
  const lastTestScores: Record<string, number> = { never: 3, over_year: 2, within_year: 1, within_3m: 0 };
  const total = Object.entries(scores).reduce((sum, [key, points]) => sum + (isYes(answers[key]) ? points : 0), 0) + (lastTestScores[String(answers.last_test)] ?? 0);
  let band = total >= 10 ? "urgent" : total >= 6 ? "elevated" : total >= 3 ? "moderate" : "low";
  if (isYes(answers.known_positive_partner) || isYes(answers.injection_sharing)) band = band === "urgent" ? band : "elevated";
  const pathway = band === "low"
  ...
  const result = { id: ++nextId, total, band, pathway: `${pathway} This is not a diagnosis or an HIV test. It only suggests next steps.`, urgent: band === "urgent", safeguardingFlag: isYes(answers.forced_sex) };
  assessments.push({ band });
  res.status(201).json(result);
});

router.get("/assessments/:id", (_req, res) => {
  res.json({ id: Number(_req.params.id), total: 5, band: "moderate", pathway: "Some risk factors. Testing and prevention services are recommended. This is not a diagnosis or an HIV test. It only suggests next steps.", urgent: false, safeguardingFlag: false });
});
...
router.get("/dashboard/summary", (req, res) => {
  ...
  const metrics = user.role === "me_officer"
    ...
    : user.role === "admin"
      ? [{ label: "Staff accounts", value: users.length, detail: "All active", href: "/admin/users", tone: "teal" }, { label: "Service points", value: services.length, detail: "3 districts", href: "/admin/directory", tone: "indigo" }, { label: "Draft content", value: 2, detail: "Awaiting review", href: "/admin/content", tone: "amber" }, { label: "Open concerns", value: safeguardingReports.filter((report) => report.status !== "closed").length, detail: "Counts only", href: "/safeguarding", tone: "rose" }]
  ...
  res.json({ metrics, recentActivity: [{ id: 1, text: "A follow-up barrier was recorded for YC-000124.", time: "42 min ago", type: "followup" }, { id: 2, text: "A referral moved to in progress.", time: "2 hrs ago", type: "referral" }, { id: 3, text: "A milestone reminder was scheduled.", time: "Yesterday", type: "eid" }], notifications });
});
...
router.post("/admin/users", (req, res) => {
  ...
  const created = { id: ++nextId, name, email: String(req.body?.email ?? `staff${nextId}@yconnect.test`), role, roleLabel: roles[role]?.label ?? "Staff member", initials: name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase(), district: String(req.body?.district ?? "Kumasi"), status: "active" };
  users.push(created);
